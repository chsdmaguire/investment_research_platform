...
export default {
    ...
    methods: {
        ...
        use_for() { return ['MACDOverlay'] },
        legend(values) {
            let xs = values.slice(1,4).map(x => {
                return x.toFixed(Math.abs(x) > 0.001 ? 4 : 8)
            })
            return [
                {value: 'Histogram: ' + xs[0], color: this.hist_colors},
                {value: 'MACD: ' + xs[1], color: this.macd_color},
                {value: 'Signal: ' + xs[2], color: this.signal_color}
            ]
        },
        calc() {
            return {
    ...
    computed: {
        ...
        hist_colors() {
            return this.sett.histColors || "#35a776"
        }
    }
}
</script>